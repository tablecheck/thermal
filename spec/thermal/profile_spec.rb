...
RSpec.describe ::Thermal::Profile do
  ...
  context 'epson_tm_t88v' do
    ...
    it 'sets variables correctly' do
      ...
      expect(obj.cjk_encoding).to eq nil
    end
    ...
    it 'finds encodings' do
      ...
      expect(obj.find_encoding('あ'.ord)).to eq nil
      expect(obj.find_encoding('気'.ord)).to eq nil
      expect(obj.find_encoding('寫'.ord)).to eq nil
      expect(obj.find_encoding('한'.ord)).to eq nil
      expect(obj.find_encoding('观'.ord)).to eq nil
    end

    it 'finds encodings with :no_cjk option' do
      expect(obj.find_encoding('A'.ord, no_cjk: true)).to eq nil
      ...
      expect(obj.find_encoding('あ'.ord, no_cjk: true)).to eq nil
      expect(obj.find_encoding('気'.ord, no_cjk: true)).to eq nil
      expect(obj.find_encoding('寫'.ord, no_cjk: true)).to eq nil
      expect(obj.find_encoding('한'.ord, no_cjk: true)).to eq nil
      expect(obj.find_encoding('观'.ord, no_cjk: true)).to eq nil
    end
